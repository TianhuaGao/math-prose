...
| `A3` | Declare type, space, or membership | `$x\in\mathcal X$`, `$x$ lies in ...`, `$F:\mathcal X\to\mathcal Y$` | Give the actual space or signature. A physical description is not a type declaration. |
...
| `A5` | Establish scope or quantification | `for every ...`, `there exists ...`, `fix ...`, `choose ...`, `for $i=1,\ldots,n$` | Preserve quantifier order and dependence. `Choose` may require a preceding existence result. |

## B. Representation and construction

| Code | Behavior | Licensed constructions | Boundary |
|---|---|---|---|
| `B1` | Supply an explicit formula | `$x$ is given by ...`, `$x$ takes the form ...`, `$x$ can be written as ...` | The object is already identified. `Can be written as` asserts an admissible representation, usually an equivalence. |
...
| `B3` | Map or transform | `maps ... to ...`, `transforms`, `pushes forward`, `pulls back`, `associates ... with ...` | State direction, domain, codomain, and frame when relevant. |
...
| `B5` | Normalize or project | `normalize`, `project onto`, `restrict to`, `embed in` | State nonzero, feasibility, uniqueness, and singular-case conditions. |
...
| `D2` | Draw a logical consequence | `implies`, `hence`, `therefore`, `we deduce`, `it follows that` | Require a valid implication from stated premises. These are not rhythm words. |
| `D3` | Establish or prove | `show`, `establish`, `prove`, `demonstrate analytically` | Match the verb to the argument. Numerical evidence does not prove a theorem. |
| `D4` | State a bound or order relation | `is bounded by`, `does not exceed`, `dominates`, `is monotone`, `satisfies` | Preserve strictness, norm, probability, uniformity, and constant dependence. |
| `D5` | Assert existence or uniqueness | `there exists`, `admits`, `has a unique`, `is well posed` | Identify the domain and hypotheses. Construction may establish existence but not automatically uniqueness. |
...
| `E4` | Optimize or select | `minimize`, `maximize`, `solve for`, `select`, `choose to satisfy` | State decision variables, objective, constraints, and existence assumptions. |
| `E5` | Converge or stabilize | `converges to`, `approaches`, `remains invariant`, `is asymptotically stable` | Preserve topology, mode of convergence, rate, locality, probability, and stability definition. |
...
| `F3` | Compare mathematical objects | `coincides with`, `differs from`, `dominates`, `is equivalent to`, `generalizes` | Name the comparison criterion and preserve direction. `Generalizes` requires recovery of the narrower case. |
| `F4` | Interpret a formula or result | `shows that`, `indicates`, `means that`, `corresponds to`, `reveals` | Do not strengthen a formal result beyond its assumptions or turn an illustration into proof. |
